...
GOption[] invervalOptions = new GOption[2];
...
public void setup() {
  ...
  invervalOptions[0] = optnIntervals;
  invervalOptions[1] = optnHourly;
...
// Start Time Test **************************************************
volatile int timeTestIndex = 0;      // Used to keep track of where we are in our testing
//volatile Colormax timeTestColormax;

// Gotta save which option is being used
// 0 = Intervals
// 1 = Hourly
volatile int selectedIntervalOption;

void startTimeTest(final Colormax inColormax){
  // Check if selected colormax is already time testing
  if(inColormax.getStatus() == inColormax.timeTesting){
   println("@@@@@@@@@@ Time testing for this unit already in progress @@@@@@@@@@");
   return;
  }
  ...
  // Get which interval option is selected
  for (int i = 0; i < invervalOptions.length ; i++) {
    if (invervalOptions[i].isSelected()) {
      selectedIntervalOption = i;
      break;
    } else if(i >= invervalOptions.length){
      println("@@@@@@@@@@ intervalOptions out of bounds @@@@@@@@@");
      return;
    }
  }
  
  inColormax.writeAlignmentOff();delay(50);
  inColormax.writeTempOn();delay(50);
...
ActionListener oneSecondTimerListener = new ActionListener() {
  public void actionPerformed(ActionEvent e) {
    ...
    if (counter < 0) {   // Check for negative values real fast
      ...
      if(selectedIntervalOption == 0 && counter >= colormaxIntervals[timeTestIndex]    // Check if we've passed our next intervals (see colormaxIntervals[])
       ||selectedIntervalOption == 1 && counter >= timeTestIndex * 7){              // OR Check if it's been an hour
        println("counter", counter);
        getAveragedReadings(inColormax);                  // Get averaged readings
        
        String prefix = "Time Test Snippet Logs/Unit_" + inColormax.getSerialNumber().substring(12, 16) + "_" + counter + "_";    // Start of file name for snippet log (Time Test Snippet Logs/ is the folder this will be in)
        String randomVal = String.valueOf((int)random(9999));  // Random number at the end of the name to avoid duplicate names
        String fileName = prefix + "(" + randomVal + ")" + ".txt";         // Put it all together w/ .txt suffix
